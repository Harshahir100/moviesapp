...
const SavedContent = () => {
    ...
    const searchDatabase = async () => {
        ...
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:5000/api/torrents/database/search', {
                params: { q: searchTerm }
            });
    ...
    const viewContent = async (tmdbId) => {
        setLoading(true);
        try {
            const response = await axios.get(`http://localhost:5000/api/torrents/content/${tmdbId}`);
    ...
    return (
        <div className="space-y-6">
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
                <h2 className="text-2xl font-bold text-white mb-4">Saved Content Library</h2>
            ...
            {results.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {results.map((item) => (
                        <div key={item.tmdb_id} className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700 hover:border-red-500 transition-all">
                            {item.poster_path && (
                                <img
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex justify-between text-xs mb-3">
                                    <span className="text-gray-400">Torrents: {item.torrent_count}</span>
                                    {item.hindi_dub_count > 0 && (
                                        <span className="text-green-400">Hindi: {item.hindi_dub_count}</span>
                                    )}
                                </div>
                                <button
                ...
                <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4 overflow-y-auto">
                    <div className="bg-gray-900 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="sticky top-0 bg-gray-900 p-4 border-b border-gray-700 flex justify-between items-center">
                            <h2 className="text-xl font-bold text-white">{selectedContent.content?.title}</h2>
                            <button
                        ...
                        <div className="p-6">
                            {/* Torrents List */}
                            <h3 className="text-lg font-semibold text-white mb-4">Available Torrents</h3>
                            <div className="space-y-3">
                                {selectedContent.torrents?.map((torrent, idx) => (
                                    <div key={idx} className="bg-gray-800 rounded-lg p-4">
                                        <div className="flex justify-between items-start">
                                            <div className="flex-1">
                                                <h4 className="text-white font-medium mb-2">{torrent.title}</h4>
                                                <div className="flex flex-wrap gap-2 mb-2">
                                                    {torrent.quality && (
                                                        <span className="px-2 py-1 bg-blue-600 rounded text-xs">{torrent.quality}</span>
                                                    )}
                                                    {torrent.is_hindi_dub && (
                                                        <span className="px-2 py-1 bg-green-600 rounded text-xs">Hindi Dubbed</span>
                                                    )}
                                                    {torrent.size && (
                                                        <span className="px-2 py-1 bg-gray-600 rounded text-xs">💾 {torrent.size}</span>
                                                    )}
                                                    <span className="px-2 py-1 bg-green-800 rounded text-xs">👥 {torrent.seeders}</span>
                                                </div>
                                                <p className="text-xs text-gray-400">Source: {torrent.source}</p>
                                            </div>
                                            <a
